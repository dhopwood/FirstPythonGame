...
# Purpose: Plays a die rolling game with 2 players. Die rolls are added to the players scores until they quit rolling or until they roll a 1, which will cause them to lose their points for that round. First one to 100 points wins the game.
# Pre-conditions: Clicks from the players to indicate their choice in the game
# Post-conditions: Die faces are displayed for the rolls, prompts are displayed when player input is needed, messages are displayed to show results of player actions such as rolling again, rolling a one, and winning the game. 
...
# function in_box
'''
Purpose: To determine if the the third parameter given (a point) is in the box determined by the first two parameters given (two points). Returns True if it is in the box, False otherwise.
Preconditions (parameters): Three point objects; assumes the first point is on the left, and the second point is on the right.
Postconditions (output/results): True if the third point is inside the box formed by the other two points. False otherwise. 
'''
